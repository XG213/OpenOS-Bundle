...
--[[
Adds all filesystems (excluding boot drive)
--]]

function addall(bootadress)
    if type(bootadress) ~= "string" then
    io.stderr:write("expected a string\n")
    return
  end
  ...
  local proxys = {}
  for a in component.list("filesystem", true) do
    if a ~= ADDRESS and component.invoke(a, "getLabel") ~= "tmpfs" and not (a:sub(1, #bootadress) == bootadress or
     component.invoke(a, "getLabel") == bootadress) then
      table.insert(proxys, component.proxy(a))
    end
  end
  if not proxys then
    io.stderr:write("cannot find filesystem(s)\n")
    return
  end
  for i,proxy in pairs(proxys) do
    ...
    if address:len() == 8 then
      ...
      for i=1, #filesystems do
        if filesystems[i].address == address then
          fs = filesystems[i]
          if type(fs.list("")) == "number" and fs.list("") > 1 and not forcefully then
            io.stderr:write(errMsg)
            return
          end
        end
...
--[[
Resets all bundle filesystems
--]]

function reset()
  for a in component.list("filesystem", true) do
    -- TODO: check to avoid errors (still works tho :3)
    remove(a)
  end
